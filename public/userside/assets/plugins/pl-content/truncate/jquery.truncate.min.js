(function($) {
    ...
    $.fn.truncate = function(options) {
        ...
        return this.each(function() {
            ...
            var $truncateWorker = $("<span/>").css(fontCSS).html(elementText).appendTo("body");
            ...
            if (originalDim > truncateDim) {
                ...
                if (options.side === "left") {
                    truncPoint = findTruncPoint(dimension, truncateDim, elementText, 0, elementText.length, $truncateWorker, options.token, true);
                    truncatedText = [options.token, elementText.slice(-1 * truncPoint)].join("")
                } else if (options.side === "center") {
                    truncateDim = parseInt(truncateDim / 2, 10) - 1;
                    truncPoint = findTruncPoint(dimension, truncateDim, elementText, 0, elementText.length, $truncateWorker, options.token, false);
                    truncPoint2 = findTruncPoint(dimension, truncateDim, elementText, 0, elementText.length, $truncateWorker, "", true);
                    truncatedText = [elementText.slice(0, truncPoint), options.token, elementText.slice(-1 * truncPoint2)].join("")
                } else if (options.side === "right") {
                    truncPoint = findTruncPoint(dimension, truncateDim, elementText, 0, elementText.length, $truncateWorker, options.token, false);
                    truncatedText = [elementText.slice(0, truncPoint), options.token].join("")
                }
                if (options.addclass) {
